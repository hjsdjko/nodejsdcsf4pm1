...
import DiaojidingdanModel from '../models/DiaojidingdanModel'
...
export default ({ config, db }) => {
	...
	api.get('/page', async (req, res) => {
		...
		try {
			...
			let where = {}
			let diaojibianhao = req.query.diaojibianhao
			if (diaojibianhao) {

				if (diaojibianhao.indexOf('%') != -1) {
					where.diaojibianhao = {
						[Op.like]: diaojibianhao
					}
				} else {
					where.diaojibianhao = {
						[Op.eq]: diaojibianhao
			...
			let diaojizhuangtai = req.query.diaojizhuangtai
			if (diaojizhuangtai) {

				if (diaojizhuangtai.indexOf('%') != -1) {
					where.diaojizhuangtai = {
						[Op.like]: diaojizhuangtai
					}
				} else {
					where.diaojizhuangtai = {
						[Op.eq]: diaojizhuangtai
					}
				}
			}
			let wanchengshijian = req.query.wanchengshijian
			if (wanchengshijian) {

				if (wanchengshijian.indexOf('%') != -1) {
					where.wanchengshijian = {
						[Op.like]: wanchengshijian
					}
				} else {
					where.wanchengshijian = {
						[Op.eq]: wanchengshijian
					}
				}
			}
			...
			let result = await DiaojidingdanModel.findAndCountAll({
				order: [orders],
				where,
				offset: (page - 1) * limit,
				limit
	...
	api.get('/lists', async (req, res) => {
		...
		try {
			let result = await DiaojidingdanModel.findAll()
	...
	api.post('/save', async (req, res) => {
		...
		try {
			...
			const userinfo = await DiaojidingdanModel.create(req.body)
	...
	api.post('/update', async (req, res) => {
		...
		try {
			...
			await DiaojidingdanModel.update(req.body, {
				where: {
				  id: req.body.id || 0
				}
			})
	...
	api.post('/delete', async (req, res) => {
		...
		try {
			...
			await DiaojidingdanModel.destroy({
				where: {
				  id: {
					[Op.in]: req.body
				  }
	...
	api.all('/info/:id', async (req, res) => {
		...
		try {
			...
			toRes.record(res, 0, await DiaojidingdanModel.findOne({ where: { id: req.params.id } }))
		} catch(err) {
	...
	api.get('/remind/:columnName/:type', async (req, res) => {
		...
		try {
			...
			if (req.params.type == 1) {
				if (req.query.remindstart) sql = "SELECT COUNT(*) AS count FROM diaojidingdan WHERE " + where + " AND " + req.params.columnName + " >= '" + req.query.remindstart + "'"
				if (req.query.remindend) sql = "SELECT COUNT(*) AS count FROM diaojidingdan WHERE " + where + " AND " + req.params.columnName + " <= '" + req.query.remindend + "'"

				if (req.query.remindstart && req.query.remindend) {
					sql = "SELECT COUNT(*) AS count FROM diaojidingdan WHERE " + where + " AND " + req.params.columnName + " >= '" + req.query.remindstart + "' AND " + req.params.columnName + " <= '" + req.query.remindend + "'"
				}
			}
			...
			if (req.params.type == 2) {
				if (req.query.remindstart) {
					...
					sql = "SELECT COUNT(*) AS count FROM diaojidingdan WHERE " + where + " AND " + req.params.columnName + " >= '" + remindStart + "'"
				}
				if (req.query.remindend) {
					let remindEnd = util.getDateTimeFormat(req.query.remindend, "yyyy-MM-dd")
					sql = "SELECT COUNT(*) AS count FROM diaojidingdan WHERE " + where + " AND " + req.params.columnName + " <= '" + remindEnd + "'"
				}

				if (req.query.remindstart && req.query.remindend) {
					let remindStart = util.getDateTimeFormat(0 + Number(req.query.remindstart), "yyyy-MM-dd")
					let remindEnd = util.getDateTimeFormat(req.query.remindend, "yyyy-MM-dd")
					sql = "SELECT COUNT(*) AS count FROM diaojidingdan WHERE " + where + " AND " + req.params.columnName + " >= '" + remindStart + "' AND " + req.params.columnName + " <= '" + remindEnd + "'"
				}
			}
	...
	api.get('/group/:columnName', async (req, res) => {
		...
		try {
			...
			// let tableName = "diaojidingdan"
			let where = " WHERE 1 = 1 "
			sql = "SELECT COUNT(*) AS total, " + columnName + " FROM diaojidingdan " + where + " GROUP BY " + columnName 
	...
	api.get('/value/:xColumnName/:yColumnName', async (req, res) => {
		...
		try {
			...
			let where = " WHERE 1 = 1 "
			if ("diaojidingdan" == "orders") {
				where += " AND status IN ('已支付', '已发货', '已完成') ";
			}

			sql = "SELECT " + xColumnName + ", SUM(" + yColumnName + ") AS total FROM diaojidingdan " + where + " GROUP BY " + xColumnName + " DESC"
