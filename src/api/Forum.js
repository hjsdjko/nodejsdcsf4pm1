...
export default ({ config, db }) => {
	...
	api.get('/page', async (req, res) => {
		...
		try {
            ...
            if (jwt.decode(req.headers.token).role != '管理员' && ForumModel.rawAttributes.hasOwnProperty('userid')) {
				where.userid = {
	...
	api.get('/list/:id', async (req, res) => {
		...
		try {
			...
			const formattedDates = list.map(item => {
				const addtime = new Date(item.addtime);
			  
				const year = addtime.getFullYear();
				const month = String(addtime.getMonth() + 1).padStart(2, '0');
				const day = String(addtime.getDate()).padStart(2, '0');
				const hour = String(addtime.getHours()).padStart(2, '0');
				const minute = String(addtime.getMinutes()).padStart(2, '0');
				const second = String(addtime.getSeconds()).padStart(2, '0');
			  
				const formattedDate = `${year}-${month}-${day} ${hour}:${minute}:${second}`;
			  
				return { ...item, addtime: formattedDate };
			  });
			let result = util.translateDataToTree(formattedDates)
			let data = {};
			result.forEach(item => {
