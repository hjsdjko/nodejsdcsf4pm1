...
import DiaojiyuyueModel from '../models/DiaojiyuyueModel'
...
export default ({ config, db }) => {
	...
	api.get('/page', async (req, res) => {
		...
		try {
			...
			let diaojishijian = req.query.diaojishijian
			if (diaojishijian) {

				if (diaojishijian.indexOf('%') != -1) {
					where.diaojishijian = {
						[Op.like]: diaojishijian
					}
				} else {
					where.diaojishijian = {
						[Op.eq]: diaojishijian
			...
			let result = await DiaojiyuyueModel.findAndCountAll({
				order: [orders],
				where,
				offset: (page - 1) * limit,
				limit
	...
	api.get('/lists', async (req, res) => {
		...
		try {
			let result = await DiaojiyuyueModel.findAll()
	...
	api.post('/save', async (req, res) => {
		...
		try {
			...
			const userinfo = await DiaojiyuyueModel.create(req.body)
	...
	api.post('/update', async (req, res) => {
		...
		try {
			...
			await DiaojiyuyueModel.update(req.body, {
				where: {
				  id: req.body.id || 0
				}
			})
	...
	api.post('/delete', async (req, res) => {
		...
		try {
			...
			await DiaojiyuyueModel.destroy({
				where: {
				  id: {
					[Op.in]: req.body
				  }
	...
	api.all('/info/:id', async (req, res) => {
		...
		try {
			...
			toRes.record(res, 0, await DiaojiyuyueModel.findOne({ where: { id: req.params.id } }))
		} catch(err) {
	...
	api.get('/remind/:columnName/:type', async (req, res) => {
		...
		try {
			...
			if (req.params.type == 1) {
				if (req.query.remindstart) sql = "SELECT COUNT(*) AS count FROM diaojiyuyue WHERE " + where + " AND " + req.params.columnName + " >= '" + req.query.remindstart + "'"
				if (req.query.remindend) sql = "SELECT COUNT(*) AS count FROM diaojiyuyue WHERE " + where + " AND " + req.params.columnName + " <= '" + req.query.remindend + "'"

				if (req.query.remindstart && req.query.remindend) {
					sql = "SELECT COUNT(*) AS count FROM diaojiyuyue WHERE " + where + " AND " + req.params.columnName + " >= '" + req.query.remindstart + "' AND " + req.params.columnName + " <= '" + req.query.remindend + "'"
				}
			}
			...
			if (req.params.type == 2) {
				if (req.query.remindstart) {
					...
					sql = "SELECT COUNT(*) AS count FROM diaojiyuyue WHERE " + where + " AND " + req.params.columnName + " >= '" + remindStart + "'"
				}
				if (req.query.remindend) {
					let remindEnd = util.getDateTimeFormat(req.query.remindend, "yyyy-MM-dd")
					sql = "SELECT COUNT(*) AS count FROM diaojiyuyue WHERE " + where + " AND " + req.params.columnName + " <= '" + remindEnd + "'"
				}

				if (req.query.remindstart && req.query.remindend) {
					let remindStart = util.getDateTimeFormat(0 + Number(req.query.remindstart), "yyyy-MM-dd")
					let remindEnd = util.getDateTimeFormat(req.query.remindend, "yyyy-MM-dd")
					sql = "SELECT COUNT(*) AS count FROM diaojiyuyue WHERE " + where + " AND " + req.params.columnName + " >= '" + remindStart + "' AND " + req.params.columnName + " <= '" + remindEnd + "'"
				}
			}
	...
	api.get('/group/:columnName', async (req, res) => {
		...
		try {
			...
			// let tableName = "diaojiyuyue"
			let where = " WHERE 1 = 1 "
			sql = "SELECT COUNT(*) AS total, " + columnName + " FROM diaojiyuyue " + where + " GROUP BY " + columnName 
	...
	api.get('/value/:xColumnName/:yColumnName', async (req, res) => {
		...
		try {
			...
			let where = " WHERE 1 = 1 "
			if ("diaojiyuyue" == "orders") {
				where += " AND status IN ('已支付', '已发货', '已完成') ";
			}

			sql = "SELECT " + xColumnName + ", SUM(" + yColumnName + ") AS total FROM diaojiyuyue " + where + " GROUP BY " + xColumnName + " DESC"
	...
	api.post('/shBatch', async (req, res) => {
		...
		try {
			...
			req.body.forEach(async id => {
				let data= await DiaojiyuyueModel.findOne({ where: { id } })
				data = data.dataValues
				data.sfsh = req.query.sfsh
				data.shhf = req.query.shhf
				await DiaojiyuyueModel.update(data, {
					where: {
						id: data.id || 0
					}
				})
